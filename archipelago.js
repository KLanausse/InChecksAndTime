 ...
 */
window.APMod = {};
APMod.server = "archipelago.gg:50592";
APMod.slot = "Siffrin";
APMod.password = "";
...
export const config = {
  ...
  settings: {
    ...
    chatTransparency: {
      title: "Transparency",
      helpMessage:
        "Set the transparency of the text chat. (Style must be set to Transparent)",
      type: "scale",
      min: 0,
      max: 100,
      step: 5,
...
[
  10, 11, 12, 13, 14, 15, 16, 17, 18, 20, 155, 156, 157, 160, 182, 183, 184,
  185, 186,
].forEach(function (id) {
  skillIdToActorName[id] = "Siffrin";
});
[32, 33, 34, 35, 36, 37, 38, 36, 40].forEach(function (id) {
  skillIdToActorName[id] = "Mirabelle";
});
[22, 23, 24, 25, 26, 27, 28, 29].forEach(function (id) {
  skillIdToActorName[id] = "Isabeau";
});
[42, 43, 44, 45, 46, 47, 48, 49, 50].forEach(function (id) {
  skillIdToActorName[id] = "Odile";
});
[52, 53, 54, 55, 56, 57, 58, 59].forEach(function (id) {
  skillIdToActorName[id] = "Bonnie";
});
...
function doDeath(slot, time, cause) {
  ...
  if (cause) {
    ...
    $gameMessage._positionType = 1;
    $gameMessage._texts[0] = format(
      deathMessages[Math.round(Math.random() * deathMessages.length) - 1],
      slot,
    );
  }
}
...
function doCommonEventHook(command) {
  ...
  console.debug(
    `Calling Event ${$dataCommonEvents[eventId].name} : ${eventId}`,
  );
  ...
  switch (eventId) {
    ...
    case 108: // Deathlink
      ...
      if ($gameSwitches._data[106]) {
        // HHM Reset
        deathReasons = [
          "{1} FEELS A TUG ON THEIR STOMACH.",
          "The curtain falls for {0}.",
          "Something's broken, failing, rotting.",
        ];

        if (($gameVariables._data[106] = 1)) {
          // HHM Reset #
      ...
      if ($gameSwitches._data[114]) {
        // Suicide Death
        deathReasons = [
          "{0} killed themselfs.",
          "{0} saved themselfs some time.",
        ];

        if ($gameVariables._data[110] <= 3) {
          // Suicide Death #
          deathReason = deathReasons[0];
        } else {
      ...
      if ($gameSwitches._data[113]) {
        // Pineapple Death
        deathReasons = [
          "{0} is allergic to pineapples.",
          "{0} is hungry still.",
        ];

        if (($gameVariables._data[109] = 0)) {
          // Pineapple Death #
      ...
      if ($gameSwitches._data[102]) {
        // Banana Death
        deathReasons = [
          "{0} slipped on a banana peel.",
          "{0} is a living comedy sketch.",
          "{0} broke their head open on a rock.",
        ];

        if ($gameVariables._data[102] <= 1) {
          // Banana Death #
          deathReason = deathReasons[0];
        } else {
          let mod = 2;
      ...
      if ($gameSwitches._data[104]) {
        // King Death
        deathReasons = [
          "The King killed {0}.",
          "The King killed {0} again.",
          "One more time.",
        ];

        if ($gameVariables._data[104] <= 1) {
          // King Death #
      ...
      if ($gameSwitches._data[105]) {
        // Tears Death
        deathReasons = [
          "A Tear froze {0} in time.",
          "{0} had a good sleep.",
          "{0} froze themselfs in time.",
        ];

        if ($gameVariables._data[105] <= 1) {
          // Tears Death #
      ...
      if ($gameSwitches._data[103]) {
        // Monster Death
        deathReasons = [
          "{0} died saving their friends.",
          "A Sadness killed {0}.",
          "Sadness tore {0} apart.",
        ];

        if ($gameVariables._data[103] <= 1) {
          // Monster Death #
      ...
      if ($gameSwitches._data[112]) {
        // First trap death
        deathReasons = [
          "{0} forgot about the trap.",
          "{0} felt safe.",
          "{0} is stupid and forgetful.",
        ];

        if ($gameVariables._data[24] <= 1) {
          // FirstTrap Death #
      ...
      if (random == 51) {
        ...
        deathReason =
          deathReasons[Math.round(Math.random() * deathReason.length)];
      }

      switch (
        $gameVariables._data[24] // !!!Kingquest
      ) {
        case 12:
      ...
      if ($gameSwitches._data[119]) {
        // FriendquestReset
        switch (
          $gameVariables._data[111] // FriendquestResets
        ) {
          case 1:
      ...
      APMod.client.deathLink.sendDeathLink(
        playerName,
        format(deathReason, playerName, playerName.toUpperCase()),
      );
      break;
    ...
    case 105: // (LOOP_PartyLvlsnSkills) Resets your party's Level 7 Skills. Lets only reset their level.
      ...
      switch (
        $gameVariables._data[134] // LoopResetSwitches
      ) {
        case 1: // Dormont
          isabeau_EXP = 104287;
    ...
    if (command) {
      ...
      switch (command.code) {
        ...
        case 121:
          ...
          if (id == 296 && operation == 1)
            APMod.client.check(APMod.BaseId.Misc + 66); // Gave Flower To The King
          break;
        ...
        case 122:
          ...
          if (id == 57 && operation == 0) {
            ...
            switch (command.parameters[4]) {
              ...
              default:
                console.warn(
                  `Sending Flower Gift Check Failed?? ${command.parameters[5]}`,
                );
                break;
            }
          }
        ...
        case 127: // Weapons
          ...
          if (operation == 0) {
            // Inc
            console.log(
              `Sending Check ${$dataItems[id].name}. | ${APMod.BaseId.Weapon + id}`,
            );
        ...
        case 128: // Armors
          ...
          if (operation == 0) {
            // Inc
            console.log(
              `Sending Check ${$dataItems[id].name}. | ${APMod.BaseId.Armor + id}`,
            );
            APMod.client.check(APMod.BaseId.Weapon + id);
          }
        ...
        case 318: // Skill
          ...
          if (operation == 0) {
            // Give
            console.log(
              `Sending Check ${$dataSkills[id].name}. | ${APMod.BaseId.Skill + id}`,
            );
...
function doFunctionHooks() {
  ...
  OrangeGreenworks.activateAchievement = function (achievementName) {
    ...
    APMod.client.check(
      APMod.BaseId.Misc + achievementNameToId[achievementName],
    );
  };
  ...
  ImageManager.loadTitle1 = function (filename, hue) {
    if (filename == "logo")
      return this.loadBitmap(
        "mod/mods/archipelago/img/titles1/",
        filename,
        hue,
        true,
      );
  ...
  Game_Message.prototype.add = function (text) {
    let formattedText = text.replace(
      ...
      async function (match, type, id) {
        // "(You {Skill1}!)"
        let newText = await APMod.Utils.createFoundItemText(
          APMod.BaseId[type],
          id,
        );
        return;
...
function doFunctionPatches() {
  ...
  Scene_Battle.prototype.commandAttack = function () {
    try {
      ...
      console.log("commandAttack errored out. Ignore.");
      console.error(e);
    }
  };
  ...
  Scene_Battle.prototype.commandGuard = function () {
    try {
      ...
      console.log("commandGuard errored out. Ignore.");
      console.error(e);
    }
  };
}
  ...
  if (require("fs").existsSync(`./www/${url}`)) {
    ...
    xhr.onload = function () {
      if (xhr.status < 400) {
        ...
        window[name] = JSONPatch.immutableJSONPatch(
          data,
...
APMod.Utils.getAct = function () {
  return $gameVariables._data[2];
};
APMod.Utils.getLoops = function () {
  return $gameVariables._data[1];
};
...
APMod.Notification.setup = function () {
  ...
  document.body.insertBefore(
    ...
    document.getElementById("ErrorPrinter"),
  );
};
  ...
  document
    ...
    .addEventListener("keydown", function (event) {
      ...
      if (event.key === "/") {
        ...
        chatWindow.classList.toggle("hide");
        if (!APMod.Chat.chatHidden) {
          APMod.Chat.chatInput.focus();
        } else if (APMod.Chat.chatInput.value == "/") {
          APMod.Chat.chatInput.value = "";
        }
      }
    });
...
APMod.Items.add = function (item) {
  ...
  if (item.item < APMod.BaseId.Item) {
    // Skill
    ...
    try {
      $gameActors._data.forEach(function (actor) {
        if (actor) if (actor._name == actorName) actor._skills.push(id);
        // actor.learnSkill();
      });
    } catch (error) {
      console.error(error);
    }
  } else if (item.item < APMod.BaseId.Weapon) {
    // Item
...
APMod.connect = async () => {
  ...
  APMod.client
    .login(window.APMod.server, window.APMod.slot, game, {
      password: window.APMod.password,
    })
    .then(() => {
    ...
    .catch((error) => {
      APMod.Notification.addNotification(
...
export const onRegister = async (mod) => {
  ...
  polyfill.src =
    "https://cdnjs.cloudflare.com/polyfill/v3/polyfill.min.js?version=4.8.0&features=structuredClone%2CArray.prototype.toSorted";
  document.body.append(polyfill);

  let iteratorPolyfill = document.createElement("script");
  iteratorPolyfill.type = "module";
  iteratorPolyfill.src =
    "https://unpkg.com/iterator-polyfill@1.0.9/dist/index.js";
  document.body.append(iteratorPolyfill);

  window.Archipelago =
    await import("https://unpkg.com/archipelago.js@2.1.0/dist/archipelago.min.js");
  window.JSONPatch =
    await import("https://cdn.jsdelivr.net/npm/immutable-json-patch/+esm");
